...
import yaml
with open('config_schnet.yaml', 'r') as file:
...
nnpot_schnet = spk.model.NeuralNetworkPotential(
...
task_schnet = spk.task.AtomisticTask(
    model=nnpot_schnet,
...
logger_schnet = pl.loggers.TensorBoardLogger(save_dir=config["logger_schnet_dir"])
...
trainer_schnet = pl.Trainer(
    callbacks=callbacks,
    logger=logger_schnet,
    default_root_dir=config["schnet_root_dir"],
    max_epochs=config["max_epochs"],
    deterministic=True,
    benchmark=False
)

trainer_schnet.fit(task_schnet, datamodule=custom_data)
...
def evaluate_model_on_test(model, test_loader, id_list):
    ...
    df.to_csv(config["test_pred_schnet_csv"], index=False)
    print(f"Test predictions saved to {config['test_pred_schnet_csv']}")
    
    return mae_test

# After training is complete, evaluate the model on the test set
test_loader = custom_data.test_dataloader()
test_mae = evaluate_model_on_test(task_schnet, test_loader, id_list)
...
# Save predictions for SchNet model
save_predictions_to_csv(task_schnet, train_loader, id_list, config["train_pred_schnet_csv"])
save_predictions_to_csv(task_schnet, custom_data.val_dataloader(), id_list, config["val_pred_schnet_csv"])
save_predictions_to_csv(task_schnet, custom_data.test_dataloader(), id_list, config["test_pred_schnet_csv"])
...
# Save and plot predictions for SchNet model
plot_actual_vs_predicted(
    config["test_pred_schnet_csv"], 
    'SchNet - Test set', 
    'schnet_actual_vs_predicted.png',  
    dpi=config["high_dpi"]
)
...
trained_model_schnet = trainer_schnet.model

plt.figure()

plt.plot(range(1, len(trained_model_schnet.train_losses) + 1), trained_model_schnet.train_losses, 
         label='Train Loss - SchNet', color='blue', linestyle='-')
plt.plot(range(1, len(trained_model_schnet.val_losses) + 1), trained_model_schnet.val_losses, 
         label='Validation Loss - SchNet', color='orange', linestyle='-')

plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.title('Loss - SchNet')
plt.legend()

plt.savefig('loss_schnet.png', dpi=config["high_dpi"])
...
plt.plot(range(1, len(trained_model_schnet.train_maes) + 1), trained_model_schnet.train_maes, 
         label='Train MAE - SchNet', color='blue', linestyle='-')
plt.plot(range(1, len(trained_model_schnet.val_maes) + 1), trained_model_schnet.val_maes, 
         label='Validation MAE - SchNet', color='orange', linestyle='-')

plt.xlabel('Epochs')
plt.ylabel('MAE')
plt.title('MAE - SchNet')
plt.legend()

plt.savefig('mae_schnet.png', dpi=config["high_dpi"])
